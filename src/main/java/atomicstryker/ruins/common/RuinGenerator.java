...
class RuinGenerator
{
    ...
    private boolean checkedCubicWorld;
    ...
    void generateNormal(World world, Random random, int xBase, int yBase, int zBase)
    {
        for (int c = 0; c < fileHandler.triesPerChunkNormal; c++)
        {
            if (random.nextFloat() * 100 < fileHandler.chanceToSpawnNormal)
            {
                createBuilding(world, random, xBase + random.nextInt(16), yBase,zBase + random.nextInt(16), false);
            }
        }
    }

    void generateNether(World world, Random random, int xBase, int yBase, int zBase)
    {
        for (int c = 0; c < fileHandler.triesPerChunkNether; c++)
        {
            if (random.nextFloat() * 100 < fileHandler.chanceToSpawnNether)
            {
                createBuilding(world, random, xBase + random.nextInt(16), yBase, zBase + random.nextInt(16), true);
            }
        }
    }

    private void createBuilding(World world, Random random, int x, int _y, int z, boolean nether)
    {
        checkForCubicWorld(world);

        final int rotate = random.nextInt(4);
        final Biome biome = world.getBiomeForCoordsBody(new BlockPos(x, isCubicWorld ? _y : 0, z));
        ...
        Integer y = findSuitableY(world, ruinTemplate, x, _y, z, nether);
        if (y != null)
        {
            if (checkMinDistance(world, ruinTemplate, ruinTemplate.getRuinData(x, y, z, rotate)))
            {
                ...
                Integer finalY = ruinTemplate.doBuild(world, random, x, y, z, rotate, false, false);
                if (finalY != null)
                {
    ...
    private void checkForCubicWorld(World world) {
    ...
    private Integer findSuitableY(World world, RuinTemplate r, int x, int _y, int z, boolean nether)
    {
        int upperLimit = _y + 16;
        int lowerLimit = _y;

        if(!isCubicWorld)
            upperLimit = WORLD_MAX_HEIGHT - 1;
